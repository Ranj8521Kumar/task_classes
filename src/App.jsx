...
async function getAnswerFromBackend({ studentClass, subject, question, imageDataUrl }) {
  const res = await fetch('/api/solve', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ studentClass, subject, question, imageDataUrl }),
  })
...
export default function App() {
  ...
  const canSubmit = useMemo(
    () => (question.trim().length > 5 || !!imageFile) && !loading,
    [question, imageFile, loading],
  )
  ...
  const askAnotherDoubt = () => clearAll()

  const copyAnswer = async () => {
    if (!steps.length) return
    const text = `Question: ${question || 'Image-based question'}\n\nSteps:\n${steps
      .map((s, i) => `${i + 1}. ${s}`)
        ...
        <div className="grid gap-4 p-4 sm:gap-6 sm:p-6 md:grid-cols-2">
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div>
              ...
              <textarea value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Example: A train travels 120 km in 2 hours. Find speed." className="h-32 w-full rounded-lg border border-slate-300 p-3 text-sm focus:border-blue-500 focus:outline-none sm:h-36" />
            </div>

            <div>
              <label className="mb-1 block text-sm font-semibold text-slate-700">Upload Question Image (optional)</label>
              <input
                type="file"
                accept="image/*"
